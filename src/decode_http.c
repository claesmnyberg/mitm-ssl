/*
  ...
  $Id: decode_http.c,v 1.3 2008-09-15 14:13:30 cmn Exp $
*/
...
#define USER_REGEX	".*account.*|.*acct.*|.*domain.*|.*login.*|" \
			...
			"id|uid|mn|mailaddress|login_username.*"
			
#define PASS_REGEX	".*pass.*|.*pw|pw.*|additional_info|.*secret.*"
